...
describe("resolveConfigDir", () => {
  it("replaces {{CONFIG_DIR}} with the provided value", async () => {
    const dir = path.join(tmp, "configdir-basic");
    await mkdir(dir, { recursive: true });
    await writeFile(
      path.join(dir, "agent.md"),
      "---\nname: agent\n---\ncfg: {{CONFIG_DIR}}\n",
    ...
    resolveConfigDir(dir, "/home/user/.config/opencode");

    const result = await readFile(path.join(dir, "agent.md"), "utf-8");
    expect(result).toContain("/home/user/.config/opencode");
    expect(result).not.toContain("{{CONFIG_DIR}}");
  });

  it("leaves files without the placeholder unchanged", async () => {
    const dir = path.join(tmp, "configdir-noop");
    await mkdir(dir, { recursive: true });
    const content = "---\nname: agent\n---\n# No placeholder\n";
    await writeFile(path.join(dir, "agent.md"), content);

    resolveConfigDir(dir, "/some/path");
  ...
  it("replaces all occurrences in a file", async () => {
    const dir = path.join(tmp, "configdir-multi");
    await mkdir(dir, { recursive: true });
    await writeFile(
      path.join(dir, "agent.md"),
      "path1: {{CONFIG_DIR}}\npath2: {{CONFIG_DIR}}\n",
    ...
    resolveConfigDir(dir, "/cfg");

    const result = await readFile(path.join(dir, "agent.md"), "utf-8");
    expect(result).toBe("path1: /cfg\npath2: /cfg\n");
  });
});
